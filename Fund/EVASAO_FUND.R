...
DadosAprovacao = data.frame(IRD,ICG,DSU,ATU,IED1,IED2,IED3,IED4,IED5,IED6,AFD1,AFD2,AFD3,AFD4,AFD5)
...
#boxplot(DadosAprovacao$TE,DadosAprovacao$TDI,DadosAprovacao$ATU,names = c("TE","TDI","ATU"), main='Evasão')
...
for (ii in 1:n) {
  ...
  #+IED1+ICG+ATU+AFD1
  ptm_rlm <- Sys.time()
  robust <- rlm(TE ~ TDI+IED1+ICG+ATU+AFD1,data=train,maxit=100)
  ...
  ptm_lm <- Sys.time()
  linear <- lm(TE~ TDI+IED1+ICG+ATU+AFD1,data=train)
  ...
  ptm_rq <- Sys.time()
  quantil <- rq(TE ~ TDI+IED1+ICG+ATU+AFD1,data=train,tau=0.5)
  ...
  ptm_svr <- Sys.time()
  support <- svm(TE ~ TDI+IED1+ICG+ATU+AFD1,data=train)#,kernel='linear'
  ...
  ptm_rs <- Sys.time()
  resistant <- ltsreg(TE ~ TDI+IED1+ICG+ATU+AFD1,data=train,method="lqs")
