...
library(readxl)
library(elasticnet)
setwd("C:\\Users\\Rafaella Souza\\Desktop\\Mestrado\\RAFAELLA_\\FUND_TOTAL")
Dados <- read_excel("TX_REPROVACAO_FUND.xlsx", sheet=1, col_names=TRUE)


TE = as.numeric(Dados$TR)
...
DadosAprovacao1 = data.frame(IRD,TDI,ICG,DSU,ATU,IED1,IED2,IED3,IED4,IED5,IED6,AFD1,AFD2,AFD3,AFD4,AFD5,TR)
...
#boxplot(DadosAprovacao$TE,DadosAprovacao$TDI,DadosAprovacao$ATU,names = c("TE","TDI","ATU"),main='Reprovação')
...
Todos_RMSE_rs = NULL
#plot(base)
file.create("resultados_erro_RF.csv")
file.create("resultados_tempo_RF.csv")
...
#ridge
  ...
  robust <- rlm(TE ~ TDI+IED1,data=train,maxit=100)
  #+TDI+IED3
  linear <- lm(TE~ TDI+IED1,data=train)
  
  quantil <- rq(TE ~ TDI+IED1,data=train,tau=0.5)
  
  support <- svm(TE ~ TDI+IED1,data=train)#,kernel='linear'
  
  resistant <- ltsreg(TE ~ TDI+IED1,data=train,method="lqs")
  ...
  write.table(Dados_Erro, file="resultados_erro_RF.csv",row.names=FALSE, col.names=FALSE, sep=";",append = TRUE)
  #write.table(Dados_Tempo, file="resultados_tempo_RF.csv",row.names=FALSE, col.names=FALSE, sep=";",append = TRUE)
...
#boxplot(las = 1, names=c('Robusta','Linear') ,
#        Todos_MAE_rlm,Todos_MAE_lm)
...
library(ggplot2)
myplot=ggplot() +
  geom_point(aes(x=DadosAprovacao$TDI , y=DadosAprovacao$TE), 
...
#t.test(Todos_MAE_ensemble, Todos_MAE_rq, alternative = "less", paired=FALSE)
...
#TesteHip_igualdade = wilcox.test(lista_ensemble, lista_lm, paired=FALSE)
#TesteHip_igualdade = wilcox.test(lista_ensemble, lista_rlm, paired=FALSE)
#TesteHip_igualdade = wilcox.test(lista_ensemble, lista_rqnc, paired=FALSE)

#TesteHip_igualdade
#Result_igualdade = ifelse (TesteHip_igualdade$p.value<0.05, 1, 0)
#print(Result_igualdade)
#print("Teste de Hipótese no Conjunto de Teste - IGUALDADE")
#print(TesteHip_igualdade$p.value)
