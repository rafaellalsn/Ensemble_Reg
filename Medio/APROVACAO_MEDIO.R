...
Dados <- read_excel("TX_REND_MEDIO_AP.xlsx", sheet=1, col_names=TRUE)

TA = as.numeric(Dados$APROVACAO)
...
#sem AFGD1
DadosAprovacao = data.frame(IRD,TDI,ICG,DSU,ATU,IED1,IED2,IED3,IED4,IED5,IED6,AFD1,AFD2,AFD3,AFD4,AFD5,TA)
...
#---------------------------MATRIZ DE CORRELAGCO----------------------#

#DadosAprovacao=unique(DadosAprovacao)

library(corrplot)
corrplot(cor(DadosAprovacao), method="square", tl.col="black", tl.srt=90, tl.cex = 0.7)
cor(DadosAprovacao)
...
#TDI+ICG+AFD1+IED1  
  ...
  ptm_rs <- Sys.time()
  resistant <- ltsreg(TA ~ TDI,data=train,method="lqs")
  prediction_resistant <- predict(resistant,newdata = test)
  resistant_prediction <- data.frame(pred = prediction_resistant, obs = test$TA)
  ...
  predictors_top<-c('LM','RQ','RLM','RS','SVR') 
  model_ensemble <- svm(TA ~ RQ+LM+RLM+RS, data=test)
  prediction_ensemble<-predict(model_ensemble,newdata = test)#[,predictors_top]
  en_prediction <- data.frame(pred = prediction_ensemble, obs = test$TA)
  ...
  write.table(Dados_Erro, file="resultados_erro_AM.csv",row.names=FALSE, col.names=FALSE, sep=";",append = TRUE)
  write.table(Dados_Tempo, file="resultados_tempoAM.csv",row.names=FALSE, col.names=FALSE, sep=";",append = TRUE)
...
library(ggplot2)
myplot=ggplot() +
  geom_point(aes(x=DadosAprovacao$TDI , y=DadosAprovacao$TA), 
