...
Dados <- read_excel("TX_REND_MEDIO_REP.xlsx", sheet=1, col_names=TRUE)

TE = as.numeric(Dados$REPROVACAO)
...
#GERAR MATRIZ DE CORRELAÇÃO CERTO
...
#plot(base)
file.create("resultados_erro_REP_M.csv")
file.create("resultados_tempo_REP_M.csv")
...
for (ii in 1:n) {
  ...
  ptm_rlm <- Sys.time()
  #TDI+ICG
  robust <- rlm(TE ~ TDI,data=train,maxit=200)
  ...
  ptm_rq <- Sys.time()
  quantil <- rq(TE ~ TDI,data=train,tau=0.5)
  ...
  ptm_svr <- Sys.time()
  support <- svm(TE ~ TDI,data=train)#,kernel='linear'
  ...
  ptm_rs <- Sys.time()
  resistant <- ltsreg(TE ~ TDI,data=train,method="lqs")
  ...
  write.table(Dados_Erro, file="resultados_erro_REP_M.csv",row.names=FALSE, col.names=FALSE, sep=";",append = TRUE)
  write.table(Dados_Tempo, file="resultados_tempo_REP_M.csv",row.names=FALSE, col.names=FALSE, sep=";",append = TRUE)
